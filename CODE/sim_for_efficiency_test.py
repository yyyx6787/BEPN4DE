...
unsim_train_data = load_data(test)
unsim_train_direction = load_data(test_dir)
unsim_train_label = load_data(test_label)
unsim_train_fea = load_data(test_feature)
...
para = int(10/90*len(normal_data))
import random

abnormal_data_part = abnormal_data[:para]

unsim_train_label = normal_label + abnormal_label[:para]
unsim_train = normal_data + abnormal_data[:para] 
unsim_train_direction = normal_dir + abnormal_dir[:para] 
unsim_train_fe = normal_fe + abnormal_fe[:para]
...
file=open(r"./data_roadnetwork/chengdu/simplification/simp_data_test","wb")
pickle.dump(whole_sim,file) #storing_list
file.close()

file=open(r"./data_roadnetwork/chengdu/simplification/simp_label_test","wb")
pickle.dump(whole_sim_label,file) #storing_list
file.close()

file=open(r"./data_roadnetwork/chengdu/simplification/simp_index_test","wb")
pickle.dump(whole_sim_index,file) #storing_list
file.close()

file=open(r"./data_roadnetwork/chengdu/simplification/simp_fe_test","wb")
pickle.dump(whole_sim_fe,file) #storing_list
file.close()

file=open(r"./data_roadnetwork/chengdu/simplification/simp_dir_test","wb")
